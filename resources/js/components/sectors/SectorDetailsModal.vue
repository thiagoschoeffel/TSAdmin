...
const props = defineProps({
  ...
  sectorId: { type: [Number, String, null], default: null },
});
...
const payload = ref(null); // structured sector data (required)

watch(() => props.modelValue, (v) => { open.value = v; if (v) tryFetch(); });
watch(open, (v) => emit('update:modelValue', v));
watch(() => props.sectorId, () => { if (open.value) tryFetch(); });
...
async function tryFetch() {
  ...
  try {
    ...
    if (!data || !data.sector) throw new Error('invalid');
    payload.value = data.sector;
  } catch (_) {
    error.value = true;
  } finally {
    loading.value = false;
...
<template>
  <Modal v-model="open" title="Detalhes do setor" size="lg" :lockScroll="true" :closeOnBackdrop="true">
    <div v-if="loading" class="space-y-6" aria-hidden="true">
      ...
      <span class="sr-only">Carregando detalhes do setor...</span>
    </div>

    <div v-else-if="error" class="flex flex-col items-center justify-center gap-3 text-center text-sm text-slate-500">
      <p class="text-sm text-rose-600">Não foi possível carregar os detalhes do setor.</p>

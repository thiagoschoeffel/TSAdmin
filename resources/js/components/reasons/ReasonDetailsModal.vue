...
const props = defineProps({
  ...
  reasonId: { type: [Number, String, null], default: null },
});
...
const payload = ref(null); // structured reason data (required)

watch(() => props.modelValue, (v) => { open.value = v; if (v) tryFetch(); });
watch(open, (v) => emit('update:modelValue', v));
watch(() => props.reasonId, () => { if (open.value) tryFetch(); });

async function tryFetch() {
  if (!props.reasonId) return;
  loading.value = true;
  error.value = false;
  payload.value = null;
  try {
    ...
    if (!data || !data.reason) throw new Error('invalid');
    payload.value = data.reason;
  } catch (_) {
    error.value = true;
  } finally {
    loading.value = false;
...
<template>
  <Modal v-model="open" title="Detalhes do Motivo" size="lg" :lockScroll="true" :closeOnBackdrop="true">
    <div v-if="loading" class="space-y-6" aria-hidden="true">
      ...
      <span class="sr-only">Carregando detalhes do motivo...</span>
    </div>

    <div v-else-if="error" class="flex flex-col items-center justify-center gap-3 text-center text-sm text-slate-500">
      <p class="text-sm text-rose-600">Não foi possível carregar os detalhes do motivo.</p>
    ...
    <div v-if="payload" class="space-y-6">
      <section class="space-y-3">
        ...
        <dl class="grid gap-4 sm:grid-cols-2">
          ...
          <div class="space-y-1">
            <dt class="text-sm font-semibold text-slate-500">Tipo de Motivo</dt>
            <dd class="text-sm text-slate-800">{{ payload.reason_type_name }}</dd>
          </div>
          <div class="space-y-1">

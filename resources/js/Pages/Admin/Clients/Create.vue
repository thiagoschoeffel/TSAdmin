...
const formatDocument = () => {
  ...
  form.document = form.person_type === 'company'
    ? applyMask(digits, '##.###.###/####-##')
    : applyMask(digits, '###.###.###-##');
};
const formatPostalCode = () => {
  form.postal_code = applyMask(form.postal_code, '#####-###');
};
const formatPhone = (key) => {
  const digits = digitsOnly(form[key]);
  const pattern = digits.length > 10 ? '(##) #####-####' : '(##) ####-####';
  form[key] = applyMask(digits, pattern);
};
    ...
    <section class="card space-y-8">
      ...
      <form @submit.prevent="submit" class="space-y-6">
          ...
          <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            ...
            <label class="form-label">
              ...
              <select v-model="form.state" required class="form-select">
                ...
                <option v-for="uf in props.states" :key="uf" :value="uf">{{ uf }}</option>
              </select>
        ...
        <div class="flex flex-wrap gap-3">
          <button type="submit" class="btn-primary" :disabled="form.processing">Salvar cliente</button>
          <Link class="btn-ghost" href="/admin/clients">Cancelar</Link>
        </div>
      </form>
    </section>
  </AdminLayout>
